...
class ShopController extends Controller
{
    public function index(): View
    {
        ...
        $prix = [
            'min' => Article::min('prix_public'),
            'max' => Article::max('prix_public')
        ];

        return view('shop', compact('articlesData', 'marques', 'familles', 'couleurs', 'prix'));
    }
    ...
    public function filter(Request $request)
    {
        ...
        if ($request->filled('prix')) {
            ...
            $query->where('prix_public', $request->input('prix'));
