...
/*************  ✨ Codeium Command ⭐  *************/
    /**
     * Handle the incoming request to filter articles by brand.
     *
     * @param Request $request The HTTP request instance containing the brand name as input.
     * @return \Illuminate\Http\JsonResponse A JSON response containing the filtered articles.
     */
/******  7ed0c73c-3e84-4da9-95a6-395e070910a6  *******/
    public function filtrerParMarque(Request $request)
    {
        $marque = $request->input('nom_marque');
        $articles = Article::where('nom_marque', $marque)->get();
    ...
    public function filtrerParFamille(Request $request)
    {
        ...
        return view('shop', compact('articles', 'familles'));
    }
