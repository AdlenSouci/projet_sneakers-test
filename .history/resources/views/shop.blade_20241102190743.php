        ...
        <form method="GET" action="{{ route('shop') }}" class="mb-4">
            <fieldset>
                ...
                <div class="form-group">
                    ...
                    <select name="marque" id="marque" class="form-control" onchange="this.form.submit()">
                        <option value="">-- Sélectionner une marque --</option>
                        @foreach($availableMarques as $marque)
                        <option value="{{ $marque }}" {{ $marque === request('marque') ? 'selected' : '' }}>{{ $marque }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="famille" id="famille" class="form-control" onchange="this.form.submit()">
                        <option value="">-- Sélectionner une famille --</option>
                        @foreach($availableFamilles as $famille)
                        <option value="{{ $famille }}" {{ $famille === request('famille') ? 'selected' : '' }}>{{ $famille }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="couleur" id="couleur" class="form-control" onchange="this.form.submit()">
                        <option value="">-- Sélectionner une couleur --</option>
                        @foreach($availableCouleurs as $couleur)
                        <option value="{{ $couleur }}" {{ $couleur === request('couleur') ? 'selected' : '' }}>{{ $couleur }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="prix" id="prix" class="form-control" onchange="this.form.submit()">
                        ...
                        <option value="0-50" {{ request('prix') == '0-50' ? 'selected' : '' }}>0 - 50 €</option>
                        <option value="50-100" {{ request('prix') == '50-100' ? 'selected' : '' }}>50 - 100 €</option>
                        <option value="100-200" {{ request('prix') == '100-200' ? 'selected' : '' }}>100 - 200 €</option>
                        <option value="200-500" {{ request('prix') == '200-500' ? 'selected' : '' }}>200 - 500 €</option>
                        <option value="500" {{ request('prix') == '500' ? 'selected' : '' }}>500+ €</option>
                    </select>
                </div>
            </fieldset>
        </form>
        ...
        <form id="filterForm">
            ...
            <div class="form-group">
                ...
                <select id="famille" name="famille" class="form-control">
                    ...
                    <option value="{{ $famille->nom_famille }}">{{ $famille->nom_famille }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            <!-- <div class="form-group">
                ...
                <input type="number" id="prix_min" name="prix_min" class="form-control" min="0" step="0.01">
            </div>
            <div class="form-group">
                <label for="prix_max">Prix Maximum</label>
                <input type="number" id="prix_max" name="prix_max" class="form-control" min="0" step="0.01">
            </div> -->
