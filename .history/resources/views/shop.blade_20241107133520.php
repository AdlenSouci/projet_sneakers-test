    ...
    <script>
        ...
        $(document).ready(function() {
            $('#filterForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: '{{ route("shop.filter") }}', // Vérifiez que la route est correctement définie
                    ...
                    success: function(response) {
                        ...
                        // Afficher les nouveaux articles filtrés
                        response.articlesData.forEach(function(article) {
                            const articleHtml = `
                    <div class="col-md-4 rounded-1">
                        <div class="card mb-4 product-wap rounded-2">
                            <div class="card rounded-3">
                                <img class="card-img rounded-0 img-fluid" src="{{ asset('img/') }}/${article.img}" alt="Product Image" />
                                <div class="card-body">
                                    <a href="{{ route('article', '') }}/${article.id}" class="h3 text-decoration-none">${article.modele}</a>
                                    <p class="text-center mb-0">${article.prix_public}</p>
                                    <p class="text-center">${article.stock} exemplaires en stock</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    `;
                            $('.row-cols-xl-4').append(articleHtml);
                        });
                    },
                    error: function(error) {
