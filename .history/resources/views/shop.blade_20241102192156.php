        ...
        <form id="filterForm">
            <div class="form-group">
                ...
                <select id="marque" name="marque" class="form-control">
                    ...
                    <option value="{{ $marque->nom_marque }}">{{ $marque->id_marque }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select id="famille" name="famille" class="form-control">
                    ...
                    <option value="{{ $famille->nom_famille }}">{{ $famille->id_famille }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select id="couleur" name="couleur" class="form-control">
                    ...
                    <option value="{{ $couleur->nom_couleur }}">{{ $couleur->id_couleur }}</option>
                    @endforeach
                </select>
            </div>
            <!-- <div class="form-group">
            ...
            $('#filterForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        response.articlesData.forEach(article => {
                            articlesHtml += `
                        <div class="col mb-5">
                            <div class="card mb-4 product-wap rounded-0">
                                <div class="card rounded-0">
                                    ...
                                    <div class="card-img-overlay rounded-0 product-overlay d-flex align-items-center justify-content-center">
                                        <ul class="list-unstyled">
                                            <li><a class="btn btn-success text-white mt-2" href="{{ url('/article/') }}/${article.id}"><i class="far fa-eye"></i></a></li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="card-body">
                                    <a href="{{ url('/article/') }}/${article.id}" class="h3 text-decoration-none">${article.modele}</a>
                                    <p class="text-center mb-0">$${article.prix_public}</p>
                                </div>
                            </div>
                        </div>
                    `;
                        ...
                        $('.row-cols-xl-4').html(articlesHtml);
                    },
                    error: function(error) {
                        console.log(error);
                    }
