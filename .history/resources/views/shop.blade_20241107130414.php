<x-app-layout>
    ...
    $('#filterForm').on('submit', function(e) {
        ...
        $('.row-cols-xl-4 .col').each(function() {
            ...
            const articlePrix = parseFloat($(this).data('prix')); // Assurez-vous que le prix est un nombre
            ...
            // Appliquer le tri prix
            if (selectedPrix === "asc") {
                if (articlePrix) {
                    $(this).show().sort(function(a, b) {
                        return articlePrix - parseFloat($(b).data('prix'));
                    });
                }
            } else if (selectedPrix === "desc") {
                if (articlePrix) {
                    $(this).show().sort(function(a, b) {
                        return parseFloat($(b).data('prix')) - articlePrix;
                    });
                }
            } else {
                $(this).show(); // Si aucun filtre prix n'est sélectionné, afficher tous
            }

            // Masquer ou afficher en fonction des autres filtres (marque et couleur)
