<x-app-layout>
    <section class="py-5 custom-section">
        ...
        <div class="container px-4 px-lg-5 mt-3"> <!-- Réduit la marge supérieure -->
    <div class="row gx-4 gx-lg-5 row-cols-1 row-cols-md-2 row-cols-xl-4 justify-content-center g-2"> <!-- Réduit l'espacement -->
        <?php foreach ($articlesData as $article) : ?>
            <?php $stock_article = array_sum(array_column($article->tailles->toArray(), 'stock')); ?>
            <div class="col mb-3" data-marque="{{ $article->nom_marque }}" data-couleur="{{ $article->nom_couleur }}" data-prix="{{ $article->prix_public }}"> <!-- Réduit la marge inférieure -->
                            ...
                            <div class="d-flex align-items-center justify-content-center mt-3">
                                <div class="me-3">
                                    ...
                                    <select id="pointure" name="pointure" class="custom-label custom-select rounded" style="width: 80px">
                                        <option value=""></option>
                                        @foreach($article->tailles as ```php
                                        $taille)
                                        @if ($taille->stock > 0)
