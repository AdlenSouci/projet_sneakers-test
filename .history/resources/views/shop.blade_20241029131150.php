<x-app-layout>
<form id="filtre-form" method="GET" action="{{ route('shop.filter') }}">
        <!-- Filtre par marque -->
        <label for="marque">Marque :</label>
        <select name="marques[]" id="marque" multiple>
        ...
        <input type="number" name="prixMin" id="prixMin" placeholder="Prix min">
        
        <label for="prixMax">Prix Max :</label>
        <input type="number" name="prixMax" id="prixMax" placeholder="Prix max">

        <!-- Filtre par famille -->
        <label for="famille">Famille :</label>
        <select name="familles[]" id="famille" multiple>
        ...
        <select name="couleurs[]" id="couleur" multiple>
    ...
    <section class="py-5 custom-section">
        <div class="container mt-5">
            <div class="row g-3">
                @forelse ($articlesData as $article)
                    <div class="col-md-3">
                        <div class="card">
                            <img src="{{ asset('img/' . $article->img) }}" class="card-img-top" alt="Product Image">
                            <div class="card-body">
                                <h5 class="card-title">{{ $article->modele }}</h5>
                                <p class="card-text">${{ $article->prix_public }}</p>
                            </div>
                        </div>
                    </div>
                @empty
                        ...
                        <div class="card mb-4 product-wap rounded-0">
                            ...
                            <div class="card-body">
                                ...
                                <ul class="list-unstyled d-flex justify-content-center mb-1">
                                    <li>
                                        <!-- Affichage de la moyenne des notes arrondie -->
                                        <span class="text-warning">
                                            {{ str_repeat('★', $article->moyenneNote) }}{{ str_repeat('☆', 5 - $article->moyenneNote) }}
                                        </span>
                                    </li>
                                </ul>
    ...
    <script>
        ...
        $('#filtre-marque').submit(function(event) {
            event.preventDefault();
            var marque = $('#marque').val();
            $.ajax({
                type: 'GET',
                url: '{{ route("filtrer-par-marque") }}',
                data: {
                    marque: marque
                },
                success: function(response) {
                    ...
                    $.each(articles, function(index, article) {
                        html += '<div>' + article.nom + '</div>';
                    });
                    $('#liste-articles').html(html);
                }
            });
        });

        $('#filtre-prix').submit(function(event) {
            event.preventDefault();
            var prixMin = $('#prixMin').val();
            var prixMax = $('#prixMax').val();
            $.ajax({
                type: 'GET',
                url: '{{ route("filtrer-par-prix") }}',
                data: {
                    prixMin: prixMin,
                    prixMax: prixMax
                },
                success: function(response) {
                    ...
                    $('#liste-articles').html(response);
                }
            });
        });
        $('#filtre-couleur').submit(function(event) {

            event.preventDefault();
            var couleur = $('#couleur').val();
            $.ajax({
                type: 'GET',
                url: '{{ route("filtrer-par-couleur") }}',
                data: {
                    couleur: couleur
                },
                success: function(response) {
        ...
        $('#filtre-famille').submit(function(event) {
            event.preventDefault();
            var famille = $('#famille').val();
            $.ajax({
                type: 'GET',
                url: '{{ route("filtrer-par-famille") }}',
                data: {
                    famille: famille
                },
                success: function(response) {
