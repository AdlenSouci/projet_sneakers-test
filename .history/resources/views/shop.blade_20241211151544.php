<x-app-layout>
    ...
    <script>
          /*script permettant d'actualiser le <span class="badge rounded-pill bg-danger">
                            {{ Cart::count() }}
                        </span> a chaque chaque article enlever ou ajouter au panier*/
        ...
        }); /* a chaque article ajouter au panier il faut que ce lien    <x-nav-link :href="route('basket')" :active="request()->routeIs('basket')">
                        {{ __('Panier') }} <span class="badge rounded-pill bg-danger">
                            {{ Cart::count() }}
                        </span>
                    </x-nav-link> dans la barre de navigation affiche dans le  {{ Cart::count() }} le nombre d'articles ajouter au panier*/
                
        if (Cart::count() > 0) {
            document.querySelector('#nbitems').textContent = "panier(" + Cart::count() + ")";
        }
