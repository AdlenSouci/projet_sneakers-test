<x-app-layout>
    ...
    $('#filterForm').on('submit', function(e) {
        ...
        // Tri des articles visibles
        if (selectedPrix === "asc") {
            $('.row-cols-xl-4 .col:visible').sort((a, b) => {
                return parseFloat($(a).data('prix')) - parseFloat($(b).data('prix'));
            }).appendTo('.row-cols-xl-4'); // Réorganiser les articles visibles par prix croissant
        } else if (selectedPrix === "desc") {
            $('.row-cols-xl-4 .col:visible').sort((a, b) => {
                return parseFloat($(b).data('prix')) - parseFloat($(a).data('prix'));
            }).appendTo('.row-cols-xl-4'); // Réorganiser les articles visibles par prix décroissant
        }
    });
});
    </script>
