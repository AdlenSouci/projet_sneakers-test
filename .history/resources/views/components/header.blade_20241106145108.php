<header class="py-5" style="background-color: #000000;">
    <div class="container px-4 px-lg-5 my-5">
        <div class="text-center text-white">
            <h1 class="display-4 fw-bolder" style="color: #de7105;">My Sneakers</h1>
            <p id="rotating-text" class="lead fw-normal text-white-50 mb-0">
                propose une large sélection de modèles, de marques et de prix.
            </p>
        </div>
    </div>
</header>

<style>
    /* Effet de transition douce pour le texte */
    #rotating-text {
        opacity: 2;
        transition: opacity 5s ease-in-out; /* Transition d'une seconde */
    }
    .fade-out {
        opacity: 0; /* État de sortie */
    }
</style>

<script>
    document.addEventListener("DOMContentLoaded", function() {
        // Textes dans l'ordre de rotation spécifique
        const texts = [
            "propose une large sélection de modèles, de marques et de prix.",
            "Que vous soyez un collectionneur averti ou un simple amateur de sneakers, vous trouverez sur notre site les chaussures qui vous correspondent.",
            "Nous proposons une large sélection de sneakers, allant des modèles classiques aux modèles les plus exclusifs. Vous trouverez sur notre site des sneakers de toutes les marques, telles que Nike, Adidas, Jordan, Converse, Puma, etc."
        ];
        
        let currentIndex = 0;
        const rotatingTextElement = document.getElementById("rotating-text");

        // Fonction pour changer le texte avec effet de transition
        function rotateText() {
            rotatingTextElement.classList.add("fade-out"); // Démarre la transition en sortie
            setTimeout(() => {
                currentIndex = (currentIndex + 1) % texts.length;
                rotatingTextElement.textContent = texts[currentIndex];
                rotatingTextElement.classList.remove("fade-out"); // Transition en entrée
            }, 1000); // Attendre que l'animation de sortie se termine (1s)
        }

        // Démarrage de la rotation toutes les 5 secondes après le premier texte
        setTimeout(() => {
            setInterval(rotateText, 5000); // Change toutes les 5 secondes
            rotateText(); // Lance immédiatement le premier changement
        }, 5000); // Attend 5 secondes avant de commencer la rotation
    });
</script>
