<x-app-layout>
  ...
  if (event.target.classList.contains('ajouter_au_panier') || event.target.closest('.ajouter_au_panier')) {
    var articleId = event.target.getAttribute('data-article-id') || event.target.closest('.ajouter_au_panier').getAttribute('data-article-id');
    var pointure = event.target.closest('.ajouter_au_panier').querySelector('#pointure').value;
    // ... reste du code ...
  }
});
                if (pointure) {
