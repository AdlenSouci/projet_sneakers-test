    ...
    <section class="py-5 custom-section">
        ...
        <div class="container">
            <form id="filterForm" class="mb-4">
                <div class="row">
                    <div class="col-md-3">
                        <select name="marque" class="form-select" id="marque">
                    ...
                    <div class="col-md-3">
                        <input type="text" id="priceRange" name="priceRange" value="0;100" data-slider-min="0" data-slider-max="100" data-slider-step="1" data-slider-value="[0,100]" />
                    </div>
                </div>
        ...
        $(document).ready(function() {
            ...
            $('#filterForm').on('submit', function(event) {
                ...
                var formData = {
                    ...
                    famille: $('#famille').val(),
                    couleur: $('#couleur').val(),
                    prix_min: slider.slider('getValue')[0],
                    prix_max: slider.slider('getValue')[1],
                };

                $.ajax({
                    url: '{{ route("filter") }}', // Assurez-vous que la route est correctement définie
                    type: 'GET',
                    data: formData,
                    success: function(response) {
                        $('#articlesContainer').empty(); // Vider les articles existants

                        response.articles.forEach(function(article) {
                            var stock_article = article.taille.reduce((acc, taille) => acc + taille.stock, 0);
